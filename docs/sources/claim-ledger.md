...
| C55 | 有效技术调研应先识别当前缺少的平台、产品、代码、运行、历史或流程证据，再选择能直接回答该层问题的来源 | [Tabloom 本地来源卡](cards/local-tabloom-research-trace.md)、[T01](../evidence/T01-Tabloom信息缺口与来源路由.md) | 2026-09-05 至 2026-09-07 | 单项目 16 阶段工程轨迹、同时期代码/测试/Git 与事后重建 | 是本地方法抽象；无对照实验，不能声称普遍提效 |
| C56 | 模型响应可同时满足传输和结构约束，却仍不满足业务目标；真实模型行为需要固定安全输入的边界实验裁决 | [T02](../evidence/T02-Tabloom真实边界与业务有效性.md) | 2026-09-06 | mock 测试、live harness、开发者边界输出与用户提供的当次 JSON | 只证明当次空分组；不外推非 Thinking 或具体模型的普遍行为 |
| C57 | 可审计 Agent 研究轨迹应保存信息缺口、来源、动作、置信度变化、决策与产物，而不声称还原模型隐藏思维过程 | [Tabloom Agent 研究轨迹](cards/local-tabloom-research-trace.md)、[决策研究日志模板](../templates/decision-research-log.yaml) | 2026-09-07 | 本地研究追踪文档及其 Reader Testing；本项目模板转译 | 记录完整不代表结论正确；原案例部分阶段为事后重建 |
| C58 | 易变事实应绑定访问日或目标版本复核；当前文档、动态统计与仓库热度不能替代同版本源码、运行快照和许可证检查 | [2026-09-08 易变事实复核](volatile-fact-check-2026-09-08.md) | 2026-09-08 | Deck.gl/Chrome/WPS 官方入口、目标 tag、GitHub API、B 站平台 API 与本机版本 | 访问日快照会过期；演示冻结前只复查高影响字段 |
