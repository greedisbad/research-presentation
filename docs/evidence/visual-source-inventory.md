...
1. **WPS 真机结果**：当前 W04 只到 mock 与构建，WPS 是主案例，不宜缺最终宿主画面。2026-09-08 已确认本机 WPS `12.1.24031` 和空插件配置，并建立[真机采集协议](runtime/wps/true-host-capture-protocol.md)；仍需用户模型端点和一次真实宿主操作。
