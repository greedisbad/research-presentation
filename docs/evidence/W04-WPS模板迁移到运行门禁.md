...
2026-09-08 已确认本机安装 WPS `12.1.24031`，但插件配置当前为空，且真机生成需要用户模型端点。完整的固定输入、截图清单与成功/失败门禁见 [`true-host-capture-protocol.md`](runtime/wps/true-host-capture-protocol.md)；在该协议执行完成前，本切片仍不得声称真机结果。
