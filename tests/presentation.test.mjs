...
test('slide content is screen copy only and starts with the selected hook', () => {
  ...
  assert.equal((slides.match(/id: '/g) ?? []).length, 24)
  assert.match(slides, /调研的终点，是下一次不用从零开始/)
  assert.match(slides, /\/demos\/cba\/index.html/)
  assert.match(slides, /\/demos\/web-excel\/index.html/)
  assert.match(slides, /\/demos\/deckgl\/index.html/)
  assert.match(slides, /\/demos\/aircraft\/index.html/)
})
